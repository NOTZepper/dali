import Image from 'next/image'
import Link from 'next/link'
import { ArrowUpRight } from 'lucide-react'

import { BookingDialogServer } from '@/components/booking-dialog-server'

export function Hero() {
  return (
    <section className="mx-auto max-w-6xl px-4 pt-10 pb-12 sm:px-6 md:px-8 md:pt-20 md:pb-24">
      <div className="grid items-center gap-8 md:grid-cols-2 md:gap-16">
        <div className="order-2 space-y-5 md:order-1 md:space-y-7">
          <p className="font-mono text-xs uppercase tracking-[0.25em] text-primary">
            Photography · Videography · Design
          </p>
          <h1 className="text-balance font-heading text-4xl font-semibold leading-[1.05] text-foreground sm:text-5xl md:text-7xl">
            A studio for the considered image.
          </h1>
          <p className="text-pretty text-sm leading-relaxed text-muted-foreground sm:text-base">
            dalisolstice is a creative studio crafting editorial photography,
            cinematic motion, and design with patience, intention, and an eye
            for light.
          </p>
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:gap-4">
            <BookingDialogServer label="Book a free call" />
            <Link
              href="/portfolio"
              className="group inline-flex items-center gap-1.5 text-sm font-medium text-foreground transition-colors hover:text-primary"
            >
              View our work
              <ArrowUpRight className="size-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
            </Link>
          </div>
        </div>

        <div className="order-1 relative aspect-[4/3] overflow-hidden rounded-md bg-muted sm:aspect-[4/3] md:order-2 md:aspect-[3/4]">
          <Image
            src="/images/hero.png"
            alt="Editorial studio photograph of a model in a flowing burgundy garment"
            fill
            priority
            sizes="(max-width: 768px) 100vw, 50vw"
            className="object-cover"
          />
        </div>
      </div>
    </section>
  )
}
